...
var thisDate = new Date(); //This variable will allow for the function to pull the current date.

var dateString = thisDate.toLocaleString(); // The .toLocaleString method will allow the time to be in local time

var dateHTML = `<h2>${dateString}</h2>`; // The `` were used in order to add an h2 element and then the variable within that element

var thisDay = thisDate.getDay(); // The variable is set to the current day of the week by using the local time which is being obtained through the thisDate variable

var eventHTML = getEvent(thisDay); // The variable eventHTML is set to a value of the function getEvent

document.getElementById('unionToday').insertAdjacentHTML('beforeend', dateHTML + eventHTML); // HTML elements will be called in before the closing tag of the uniounToday id element and it will change based upon the day of the week.
